...
export function renderHtml(spec, cfg, photoSrc = (p) => p, { fragment = false } = {}) {
  ...
  const css = `
  .tt { font-family: "Century Gothic", "Avenir Next", "Helvetica Neue", Arial, sans-serif; color:#333; background:#fff; width:880px; padding:18px 0; box-sizing:content-box; }
  .tt .tt-hdr { display:flex; border:2px solid #333; }
  .tt .tt-hdr .tt-l { flex:1; padding:8px 22px; border-right:2px solid #333; }
  .tt .tt-hdr h1 { font-weight:300; font-size:40px; letter-spacing:1px; margin:0; color:#555; }
  .tt .tt-hdr h2 { font-weight:800; font-size:22px; margin:6px 0 0; }
  .tt .tt-hdr .tt-k { padding:10px 14px; min-width:260px; }
  .tt .tt-hdr .tt-k b { display:block; font-size:22px; margin-bottom:6px; }
  .tt .tt-chips { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; }
  .tt .tt-chip { color:#fff; font-weight:800; font-size:11px; text-align:center; padding:6px 4px; }
  ...
  .tt .tt-bar { color:#fff; font-weight:800; font-size:18px; text-align:center; padding:7px; }`;
